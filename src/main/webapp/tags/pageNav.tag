...
<ul class="pagination justify-content-center">
	...
	<li data-page="${pageObject.startPage -1 }" class="page-item"><c:if
			test="${pageObject.startPage > 1 }">
			<a
				href="${listURI }?page=${pageObject.startPage - 1 }&${pageObject.notPageQuery}&${perGroupPageNum}&${query}"
				class="page-link" title="click to move previous page group!"
				${tooltip }> <i class="fa fa-angle-left"></i>
			</a>
		</c:if> <c:if test="${pageObject.startPage == 1 }">
			<a href="" onclick="return false" ${noMove } ${tooltip }
				class="page-link"> <i class="fa fa-angle-left"
				style="color: ${noLinkColor};"></i>
			</a>
		</c:if></li>
	<c:forEach begin="${pageObject.startPage }"
		...
		<li ${(pageObject.page == cnt)?"class=\"active \"":"" }
			...
			<!-- 페이지와 cnt가 같으면 링크가 없음 --> <c:if test="${pageObject.page == cnt }">
				<a href="" onclick="return false" class="page-link" ${noMove }
					${tooltip }>${cnt} </a>
			</c:if> <!-- 페이지와 cnt가 같지 않으면 링크가 있음 --> <c:if
				test="${pageObject.page != cnt }">
				<a
					href="${listURI }?page=${cnt }&${pageObject.notPageQuery}${query}"
					title="click to move ${cnt } page" ${tooltip } class="page-link">${cnt}
				</a>
			</c:if>
		</li>
	</c:forEach>
	<li data-page=${pageObject.endPage + 1 } class="page-item"><c:if
			test="${pageObject.endPage < pageObject.totalPage }">
			<a
				href="${listURI }?page=${pageObject.endPage + 1 }&${pageObject.notPageQuery}&${perGroupPageNum}&${query}"
				title="click to move next page group!" ${tooltip } class="page-link">
				<i class="fa fa-angle-right"></i>
			</a>
		</c:if></li>
	<c:if test="${pageObject.endPage == pageObject.totalPage }">
		<li data-page="${pageObject.endPage + 1 }" class="page-item"><a
			href="" onclick="return false" ${noMove } ${tooltip }
			class="page-link"> <i class="fa fa-angle-right"
				style="color: ${noLinkColor};"></i>
		</a></li>
	</c:if>
	<li data-page=${pageObject.totalPage }><c:if
			test="${pageObject.page < pageObject.totalPage }">
			<a
				href="${listURI }?page=${pageObject.totalPage }&${pageObject.notPageQuery}&${perGroupPageNum}&${query}"
				title="click to move last page!" ${tooltip } class="page-link">
				<i class="fa fa-angle-double-right"></i>
			</a>
		</c:if></li>
	<li data-page="${pageObject.totalPage }" class="page-item"><c:if
			test="${pageObject.page == pageObject.totalPage }">
			<a href="" onclick="return false" ${noMove } ${tooltip }
